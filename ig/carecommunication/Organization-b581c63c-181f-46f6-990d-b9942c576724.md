# 1st message - Example of a sender organization with a SOR and an EAN identifier. - DK MedCom Carecommunication v5.0.2

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **1st message - Example of a sender organization with a SOR and an EAN identifier.**

## Example Organization: 1st message - Example of a sender organization with a SOR and an EAN identifier.

Profile: [MedComMessagingOrganization](http://medcomfhir.dk/ig/messaging/4.0.2/StructureDefinition-medcom-messaging-organization.html)

**identifier**: `https://www.gs1.org/gln`/5790000209354, `urn:oid:1.2.208.176.1.1`/265161000016000

**name**: Hjerteafdelingen på Herlev og Gentofte hospital



## Resource Content

```json
{
  "resourceType" : "Organization",
  "id" : "b581c63c-181f-46f6-990d-b9942c576724",
  "meta" : {
    "profile" : [
      "http://medcomfhir.dk/ig/messaging/StructureDefinition/medcom-messaging-organization"
    ]
  },
  "identifier" : [
    {
      "system" : "https://www.gs1.org/gln",
      "value" : "5790000209354"
    },
    {
      "system" : "urn:oid:1.2.208.176.1.1",
      "value" : "265161000016000"
    }
  ],
  "name" : "Hjerteafdelingen på Herlev og Gentofte hospital"
}

```
